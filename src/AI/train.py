...
def train(data):
    ...
    cat_cols=['Gender', 'Marital_Status','Ocupation']
    new_df = cat_cols + num_columns

    categorical_columns = get_categorical_data(gender, marital, ocupation)
    numeric_columns = get_numeric_data(age, grad_period, children, s1, a2, d3, a4, d5, s6, a7, s8, a9, d10, s11, s12, d13, s14, a15, d16, d17, s18, a19, a20, d21)
    ...
    centroids = cluster_analysis(kmeans_model, normalized_data)
    dump(denormalized_clusters,open('cluster_description_joined_forms.pkl','wb'))
    dump(normalized_data, open('normalized_data_joined_forms.pkl','wb'))
    pkl_file = pd.read_pickle('cluster_description_joined_forms.pkl')
    pkl_file.to_csv('clusters_description_joined_forms.csv')
    pkl_file = pd.read_pickle('normalized_data_joined_forms.pkl')
    pkl_file.to_csv('normalized_data_joined_forms.csv')
